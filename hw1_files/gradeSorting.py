...
if(len(sys.argv) < 3):
    print("Usage: python gradeSorting.py paragraphFilename.txt wordListFilename.txt [--valgrind] [--verbose] [--bsearch]")
...
cmd = ''
if useVerbose:
    cmd = f"./text-indexing 1 {paraFile} {wordListFile}"
else:
    cmd = f"./text-indexing 0 {paraFile}  {wordListFile}"
...
words = []
with open(paraFile, 'r') as inputFile:
    print(f"Testing with file {paraFile}...")
    words = re.split(r"[ ,\.\?!()-/\n]", inputFile.readline())
    while '' in words:
        words.remove('')
...
# wordList = []
# with open(wordListFile, 'r') as inputFile2:
#     wordList = inputFile2.read().strip()
...
if f"mode: {mode}" not in line or f"paragraph: {paraFile}" not in line or f"words: {wordListFile}" not in line:
    print("Incorrect top line. Do not modify the code provided. Test failed. ❌")
    print(f"Expected: mode: {mode} | paragraph: {paraFile} | words: {wordListFile}")
